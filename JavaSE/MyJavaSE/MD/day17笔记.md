...
* A:画图演示
	* 画图说明比较过程
* B:代码优化
	* 为了减少比较，优化hashCode()代码写法。  //尽量让不同属性的hashCode()值不同，少去调用equals
	* 最终版就是自动生成即可。
	public int hashCode() {
		final int prime = 31;
		int result = 1;
...
* 使用Scanner从键盘读取一行输入,去掉其中重复字符, 打印出不同的那些字符
	* aaaabbbcccddd
			...
			for(char c : arr) {		 //遍历字符数组   Character也可以。why
...
###17.09_集合框架(TreeSet存储Integer类型的元素并遍历)
	...
	* TreeSet是用来排序的,并且可以保证元素的唯一.

###17.10_集合框架(TreeSet存储自定义对象) 
* A:案例演示
    * 存储Person对象 , 实现Comparable接口
    * 当compareTo方法返回0时,集合中只有一个元素.
    * 反回正数时,怎么存怎么取
    * 返回负数时,倒序输出
    *  二叉树: 小的存储在左边(负数) ; 大的存储在右边(正数) ;相等(0)就不存.
...
* 在一个集合中存储了无序并且重复的字符串,定义一个方法,让其有序(字典顺序),而且还不能去除重复
			 ...
			 * 2,List<String> list
			 */
			public static void sort(List<String> list) {  //给了泛型
