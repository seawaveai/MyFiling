...
* A:二进制的数据表现形式
	* 由0,1组成。以0b(b可以大写也可以小写)开头(JDK1.7版本开始可以表示二进制字面值)。
* B:八进制的数据表现形式
	* 由0,1,…7组成。以0开头
* C:十进制的数据表现形式
	* 由0,1,…9组成。  整数默认是十进制的 
...
###02.04_Java语言基础(任意进制到十进制的转换图解)(了解) /余数倒序排列
* A:任意进制到十进制的转换原理
	* 系数：就是每一位上的数据。
	* 基数：X进制，基数就是X。
...
* C:基本数据类型分类(4类8种) 
	* 整数型
		* byte 占一个字节  -128到127    //byte数据类型表示-128的理解
		* short 占两个字节  -2^15~2^15-1
		* int 占四个字节 -2^31~2^31-1     $ 整数默认类型是int
		* long 占八个字节 -2^63~2^63-1    $若赋值给long要在后面加l或者L,不然默认为int类型  主要区分1 L l I i
	* 浮点型
		* float 占4个字节   -3.403E38~3.403E38
		* double 占8个字节  -1.798E308~1.798E308   $小数默认的类型是double类型
	* 字符型
		* char 占两个字节 0~65535(2^16-1)
	* 布尔型
		* boolean 
		* 在java中不可以运算,但在C语言中可以运算 0代表false,非零代表true
...
* 进行混合运算的时候,byte,short,char不会相互转换,都会自动类型提升为int类型,其他类型进行混合运算的是小的数据类型提升为大的
	...
	* B:float表示的数据范围比long的范围要大 
		...
		* 1.4E-45~3.4E+38;E代表是以10为底，后面的是指数。
...
* C:案例演示
	...
	* System.out.println('a'+1+"hello");       98hello     $任何数据类型用+与字符串连接都会产生新的字符串$
* D:                                                       $+在有字符串参与中被称为字符串连接符
	* System.out.println("5+5="+5+5);
	* System.out.println(5+5+"=5+5");
	
###02.18_Java语言基础(char数据类型)(掌握)
* A:char c = 97;	0到65535  (2^32-1)
...
* C:算数运算符有哪些
	* +, -, *, /, %, ++, -- 
* D:注意事项：
	* a:+,代表正号,做加法运算,字符串的连接符            System.out.println(10 / 3);
	* b:整数相除只能得到整数。如果想得到小数，必须把数据变化为浮点数类型   System.out.println(10 / 3.0);
	* c:/获取的是除法操作的商，%获取的是除法操作的余数             System.out.println(3 % 5);
	* %运算符
...
* A:案例演示
	...
	* 
			...
			a = b++;       $当++在变量后面时,会先将变量中的值取出做赋值操作,然后再自加1		
			c = --a;	   $当++在变量前面时,会先自加1,然后赋值操作
			b = ++a;		
			a = c--;			
...
* C:面试题
	* byte b = 10;
	* b++;         // 默认转换了 b = (byte) (b + 1)
	* b = b + 1;   // byte与int类型相加,先提升到int类型后相加,
	* * 问哪句会报错,为什么 
