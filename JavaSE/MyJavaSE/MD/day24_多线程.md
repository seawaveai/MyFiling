...
* A:Java程序运行原理
	* Java命令会启动java虚拟机JVM，等于启动了一个应用程序(一个进程),该进程会自动启动一个 “主线程” ，然后主线程去调用某个类的 main 方法。	
* B:JVM的启动是多线程的吗
	* JVM启动至少启动了垃圾回收线程和主线程，所以是多线程的。


class MyThread extends Thread implements Runnable {  //class前面不能有public，只有有main的类才放public
	public void run() {
...
* 2.设置名字( 第一种开启线程的设置名字两种方式:)
	* 1通过构造函数可以传入String类型的名字
	* 
			new Thread("xxx") {
				public void run() {
	...
	* 2通过setName(String)方法可以设置线程对象的名字
	* 
...
* Thread.currentThread()协助第二种方法获取当前线程的名字,以及设置名字; 
*
			new Thread(){  //可以通过构造方法设置名字
				public void run(){
					System.out.println(getName()+"第一种开启线程");
					}
				}.start();
			
			new Thread(new Runnable() { //不能通过构造方法设置名字
				public void run() {
					Thread.currentThread().setName("222"); //这样才可以设置名字
					for(int i = 0; i < 1000; i++) {
...
* join(int), 可以指定等待时间毫秒值，之后再继续交替执行
	* 
...
* 使用synchronized关键字修饰一个方法, 该方法中所有的代码都是同步的 //同步方法只需要在方法上加synchronized关键字即可
			/*
			 * 非静态同步函数的锁是:this   要创建引用并用final修饰
			 * 静态的同步函数的锁是:字节码对象   类名.调用即可
			 */
		class Printer {
...
* 使用同步技术可以解决这种问题, 把操作数据的代码进行同步, 不要多个线程一起操作
			...
			class TicketsSeller extends Thread {
				...
				static Object obj = new Object(); //如果用引用数据类型成员变量当作锁对象,必须是静态的
...
###24.19_多线程(火车站卖票的例子用实现Runnable接口)(掌握)
		   MyTicket mt = new MyTicket();
				...
				T1.start();
				T1.start();
				T1.start();  //IllegalThreadStateException 多次启动一个线程是非法的??
				*/	
